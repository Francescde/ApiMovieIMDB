...
class TestMovieListResource(TestCase):
    ...
    def test_get_movies_with_invalid_page_size(self):
        # Arrange
        genre = Genre(name='Action')
        db.session.add(genre)
        db.session.commit()
        ...
        # Act
        client = self.app.test_client()
        response = client.get('/movies?page_size=invalid_size')

        # Assert
        self.assertEqual(response.status_code, 400)
        data = response.json
        self.assertEqual(data['message'], 'page size value is invalid')
